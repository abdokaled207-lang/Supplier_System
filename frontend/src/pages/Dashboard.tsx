...
import type { BestSeller, DashboardStats } from "../api/types";
...
export function Dashboard() {
  const dashboard = useQuery({
    queryKey: ["report-dashboard", LOW_STOCK_THRESHOLD],
    queryFn: () => api.get<{ data: DashboardStats }>(`/reports/dashboard?lowStockThreshold=${LOW_STOCK_THRESHOLD}`),
  });
  ...
  const isLoading = dashboard.isLoading || bestSellers.isLoading;
  const isError = dashboard.isError || bestSellers.isError;
  ...
  const statsData = dashboard.data?.data;
  const today = statsData?.todaySales;
        ...
        <table>
          ...
          <tbody>
            {["pending", "processing", "shipped", "delivered", "cancelled"].map((status) => {
              const count = statsData?.statusCounts[status.toUpperCase()] ?? 0;
